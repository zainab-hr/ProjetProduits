...
const ClientDashboard = () => {
  ...
  const isFemme = user?.genre === 'FEMME';
  ...
  const loadProducts = async () => {
    try {
      ...
      if (isHomme) {
        ...
        combined = [...hommeProducts.slice(0, hommeCount), ...femmeProducts.slice(0, femmeCount)];
      } else if (isFemme) {
        // 80% femme, 20% homme
        const femmeCount = Math.ceil(femmeProducts.length * 0.8);
        const hommeCount = Math.ceil(hommeProducts.length * 0.2);
        combined = [...femmeProducts.slice(0, femmeCount), ...hommeProducts.slice(0, hommeCount)];
      } else {
        // 50/50 for other users
        combined = [...hommeProducts, ...femmeProducts];
      }
  ...
  return (
    <div className="client-dashboard">
      ...
      <div className="hero-section" style={{ background: themeGradient }}>
        <div className="hero-content">
          <div className="hero-badge">
            <i className={`fas fa-${isHomme ? 'mars' : 'venus'}`}></i>
            Collection {isHomme ? 'Homme' : 'Femme'}
          </div>
          <h1>Bienvenue, {user?.username}!</h1>
          <p>Découvrez notre sélection exclusive de produits pour vous</p>
        ...
        <div className="hero-decoration">
          <i className={`fas fa-${isHomme ? 'tshirt' : 'gem'} hero-icon`}></i>
        </div>
      </div>
      ...
      <style jsx="true">{`
        ...
        .product-card-client:hover .product-overlay {
          opacity: 1;
        }
