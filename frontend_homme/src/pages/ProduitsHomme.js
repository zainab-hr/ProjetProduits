...
const ProduitsHomme = () => {
  ...
  const loadProducts = async () => {
    try {
      const response = await HommeService.getAllProduits();
  ...
  const handleDelete = async (product) => {
    if (window.confirm(`Êtes-vous sûr de vouloir supprimer "${product.nom}"?`)) {
      try {
        await HommeService.deleteProduit(product.id);
  ...
  const handleView = async (product) => {
    try {
      ...
      // This would need a user context - simplified for demo
  ...
  const handleSubmit = async (formData) => {
    try {
      if (selectedProduct) {
        await HommeService.updateProduit(selectedProduct.id, formData);
        toast.success('Produit modifié avec succès');
      } else {
        await HommeService.createProduit(formData);
    ...
    <div className="container-fluid py-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="text-muted mb-0">Gérez les produits de la section Homme</p>
        </div>
